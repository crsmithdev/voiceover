...
const DEFS: PromptDef[] = [
  ...
  caller(
    ...
    [
      ...
      "- Never give a payment detail, a card number, a date of birth, an account number or a government number unless it is listed as releasable above. A field that says Chris holds it is one you do not have.",
      "- Never agree to a price, a fee or a commitment that is not in the goal.",
      "- Every time you say you do not have a detail, or you withhold one, or you refuse a price, a fee or a commitment: call the note_deferred_detail tool first, with one short line naming what was asked. Then answer the person. Chris only learns what was asked from that tool.",
      "{limits}",
  ...
  caller(
    "tool.note-deferred",
    "Tool: note_deferred_detail",
    "The description the caller's brain reads for the tool that records a detail it could not give (spec 9.8).",
    "Record one thing you were asked for and did not give, or a price or commitment you refused. Chris cannot hear this call, so this tool is the only way he learns what was asked. Call it the moment it happens, before you answer the person, and then carry on.",
  ),
  caller(
    "style",
    "How to speak",
